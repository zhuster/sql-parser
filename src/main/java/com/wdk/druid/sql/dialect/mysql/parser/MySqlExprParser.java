...
import com.wdk.druid.sql.ast.statement.SQLForeignKeyImpl.Option;
...
import com.wdk.druid.sql.dialect.mysql.ast.expr.MySqlMatchAgainstExpr.SearchModifier;
...
public class MySqlExprParser extends SQLExprParser {
    ...
    static {
        ...
        AGGREGATE_FUNCTIONS_CODES = FnvHash.fnv1a_64_lower(strings, true);//将字符串转换为hashcode存放在数组中，并排序
        AGGREGATE_FUNCTIONS = new String[AGGREGATE_FUNCTIONS_CODES.length];
        for (String str : strings) {
            long hash = FnvHash.fnv1a_64_lower(str);//再次获取字符串的hashcode
            int index = Arrays.binarySearch(AGGREGATE_FUNCTIONS_CODES, hash);//找到索引
            AGGREGATE_FUNCTIONS[index] = str;//这样就可以根据hashcode的大小将字符串放到数组中
        }
    }

    public MySqlExprParser(Lexer lexer){
        super(lexer, JdbcConstants.MYSQL);
        this.aggregateFunctions = AGGREGATE_FUNCTIONS;//todo 连续两次赋值为了兼容而变得冗余
        this.aggregateFunctionHashCodes = AGGREGATE_FUNCTIONS_CODES; //聚集函数的hash值数组
    }

    public MySqlExprParser(String sql){
        this(new MySqlLexer(sql));
        this.lexer.nextToken();//父类初始化了lexer
    }
