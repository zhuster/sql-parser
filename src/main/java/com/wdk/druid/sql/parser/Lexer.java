...
import static com.wdk.druid.sql.parser.CharTypes.isFirstIdentifierChar;
import static com.wdk.druid.sql.parser.CharTypes.isIdentifierChar;
import static com.wdk.druid.sql.parser.CharTypes.isWhitespace;
import static com.wdk.druid.sql.parser.LayoutCharacters.EOI;
import static com.wdk.druid.sql.parser.SQLParserFeature.KeepComments;
import static com.wdk.druid.sql.parser.SQLParserFeature.OptimizedForParameterized;
import static com.wdk.druid.sql.parser.SQLParserFeature.SkipComments;
import static com.wdk.druid.sql.parser.Token.COLONCOLON;
import static com.wdk.druid.sql.parser.Token.COLONEQ;
import static com.wdk.druid.sql.parser.Token.COMMA;
import static com.wdk.druid.sql.parser.Token.DOT;
import static com.wdk.druid.sql.parser.Token.EOF;
import static com.wdk.druid.sql.parser.Token.EQ;
import static com.wdk.druid.sql.parser.Token.ERROR;
import static com.wdk.druid.sql.parser.Token.LBRACE;
import static com.wdk.druid.sql.parser.Token.LBRACKET;
import static com.wdk.druid.sql.parser.Token.LITERAL_ALIAS;
import static com.wdk.druid.sql.parser.Token.LITERAL_CHARS;
import static com.wdk.druid.sql.parser.Token.LPAREN;
import static com.wdk.druid.sql.parser.Token.RBRACE;
import static com.wdk.druid.sql.parser.Token.RBRACKET;
import static com.wdk.druid.sql.parser.Token.RPAREN;
...
import com.wdk.druid.util.StringUtils;
...
public class Lexer {
    ...
    public void buildSql(Node node) {
        this.sqlBuilder.build(node);
    }

    public void buildSqlSimple(String val) {
        Node node = new Node(val);
        sqlBuilder.build(node);
    }
    ...
    public void buildSqlExpr(SQLExpr sqlExpr) {
        if (sqlExpr instanceof SQLIdentifierExpr) {
            ...
            if (owner instanceof SQLIdentifierExpr) {
                ...
                String ownerName = identifierExpr.getName();
                node.setOwner(ownerName);
            }
            buildSql(node);
            sqlBuilder.addSqlExpr(sqlExpr);
            ...
            //如果left与right是SQLBinaryOpExpr则递归调用
            SQLBinaryOpExpr binaryOpExpr = (SQLBinaryOpExpr) sqlExpr;
            buildSqlExpr(binaryOpExpr.getLeft());
            SQLBinaryOperator operator = binaryOpExpr.getOperator();
            buildSqlSimple(operator.getName());
            buildSqlExpr(binaryOpExpr.getRight());
        }
        //todo 还有其它类型的需要加上
    }
     ...
     */
    public void buildSqlTable(SQLTableSource sqlTableSource) {
        if (sqlTableSource instanceof SQLExprTableSource) {
            sqlBuilder.addTableSource((SQLExprTableSource) sqlTableSource);
        }
        //todo 添加其它类型
    }

    /**
     * 添加 sqlTableSource or sqlExpr
     *
     * @param sqlObject
     */
    public void buildSqlObject(SQLObject sqlObject) {
        if (sqlObject instanceof SQLExpr) {
            buildSqlExpr((SQLExpr) sqlObject);
        } else if (sqlObject instanceof SQLTableSource) {
            buildSqlTable((SQLTableSource) sqlObject);
        }
    }
    ...
    public Lexer(String input, boolean skipComment) {
        this.skipComment = skipComment;//是否忽略注释

        this.text = input;//记录sql
        this.pos = 0;//初始化指针的初始位置
        ch = charAt(pos);//获取指针当前的字符
    }
    ...
    public final void nextToken() {
        ...
        for (; ; ) {
            if (isWhitespace(ch)) { //根据char的asc码范围判断是否为空字符
                if (ch == '\n') {
                    line++;

                    lines = line - startLine; //统计行数
                }
            ...
            if (ch == '$' && charAt(pos + 1) == '{') { //占位符，todo 这部分是对mybatis的支持吗？
                scanVariable();
                return;
            }
