...
import com.wdk.druid.sql.ast.statement.SQLExprHint;
...
import com.wdk.druid.sql.ast.statement.SQLValuesTableSource;
...
public class SQLSelectParser extends SQLParser {
    ...
    public SQLSelectParser(SQLExprParser exprParser, SQLSelectListCache selectListCache){
        super(exprParser.getLexer(), exprParser.getDbType()); //todo 反复重复赋值
    ...
    public SQLSelect select() {
        ...
        if (lexer.token == Token.WITH) { //这部分应该是oracle的
    ...
    public SQLSelectQuery queryRest(SQLSelectQuery selectQuery, boolean acceptUnion) {
        ...
        if (lexer.token == Token.UNION) {
            do {
                ...
                if (lexer.token == Token.ALL) {
                    ...
                    lexer.buildSqlSimple("ALL");
                    lexer.nextToken();
                } else if (lexer.token == Token.DISTINCT) {
                    union.setOperator(SQLUnionOperator.DISTINCT);
                    lexer.nextToken();
                    lexer.buildSqlSimple("DISTINCT");
                }
        ...
        if (lexer.token == Token.INTERSECT) {
            lexer.nextToken();
            lexer.buildSqlSimple("INTERSECT");
...
//            where = this.exprParser.expr();
        ...
        for (;;) {
            ...
            if(sqlExpr instanceof SQLPropertyExpr){
              ...
              SQLExpr owner = propertyExpr.getOwner();
               ...
               lexer.sqlBuilder().build(node);
               lexer.sqlBuilder().addSqlExpr(sqlExpr);
            }

            if(sqlExpr instanceof SQLIdentifierExpr){
                SQLIdentifierExpr identifierExpr = (SQLIdentifierExpr)sqlExpr;
                SqlBuilder sqlBuilder = lexer.sqlBuilder();
                sqlBuilder.build(new Node(identifierExpr.getName()));
                sqlBuilder.addSqlExpr(identifierExpr);
            }
    ...
    protected void parseTableSourceQueryTableExpr(SQLExprTableSource tableReference) {
        if (lexer.token == Token.LITERAL_ALIAS || lexer.token == Token.IDENTIFIED //todo 暂时先不考虑
