...
import com.wdk.druid.sql.ast.statement.SQLColumnCheck;
import com.wdk.druid.sql.ast.statement.SQLColumnDefinition;
import com.wdk.druid.sql.ast.statement.SQLColumnPrimaryKey;
import com.wdk.druid.sql.ast.statement.SQLColumnReference;
import com.wdk.druid.sql.ast.statement.SQLColumnUniqueKey;
...
import com.wdk.druid.sql.ast.statement.SQLNotNullConstraint;
import com.wdk.druid.sql.ast.statement.SQLNullConstraint;
import com.wdk.druid.sql.ast.statement.SQLPrimaryKey;
import com.wdk.druid.sql.ast.statement.SQLPrimaryKeyImpl;
    ...
    public SQLExpr expr() {
        if (lexer.token == Token.STAR) {
            ...
            if (lexer.token == Token.DOT) {
                lexer.nextToken();
                accept(Token.STAR);
                SQLPropertyExpr propertyExpr = new SQLPropertyExpr(expr, "*");
                lexer.buildSqlExpr(propertyExpr);
                return propertyExpr;
            }
...
//                        lexer.buildSqlSimple(",");
                        lexer.nextToken();
//                        lexer.buildSqlSimple(lexer.token().name);
...
//                    lexer.buildSqlSimple(ident);
                } else {
                    sqlExpr = new SQLIdentifierExpr(ident, hash_lower);
//                    lexer.buildSqlExpr(sqlExpr);
                }
                break;
            case NEW:
...
//                lexer.buildSqlSimple(tok.name);
    ...
    public void parseHints(List hints) {
        if (lexer.token == Token.HINT) {
            ...
            lexer.sqlBuilder().build(new Node(lexer.stringVal()));
    ...
    public SQLSelectItem parseSelectItem() {
        ...
        if (token == Token.IDENTIFIER) {
            ...
            if (hash_lower == FnvHash.Constants.CONNECT_BY_ROOT) { //应该是oracle的
            ...
            if (token == Token.DOT) {
                ...
                if (token == Token.LPAREN) {
                    ...
                    if (name_hash_lower == FnvHash.Constants.NEXTVAL) { //应该也是oracle的
                        expr = new SQLSequenceExpr((SQLIdentifierExpr) expr, SQLSequenceExpr.Function.NextVal);
                    } else if (name_hash_lower == FnvHash.Constants.CURRVAL) { //oracle的
